    ...
    <table class="table table-striped table-bordered table-hover"  >
        ...
        <c:forEach var="users" items="${users}">
            <tr>
                ...
                <td align="center">${users.pt}</td>
                <td align="center">
                    &nbsp;&nbsp;&nbsp;      
                    <a class="btn btn-danger active btn-xs" onclick="return delete_alert()"  href="${pageContext.request.contextPath}/user/delete.do?id=${users.id}">删除</a>
                   <a class="btn btn-info active btn-xs" href="${pageContext.request.contextPath}/user/updateselect.do?id=${users.id}">修改</a>             
                </td>
            </tr>
        </c:forEach>
        </tr>
...
<div style="width: 79%;height:2px;">
   <!--页数显示-->
      所有员工记录<font size="3" color="red">${pageUtil.pageNumber}</font>条,当前第<font size="3" color="red">${pageUtil.pageIndex}</font>页
</div>
...
<div style="width: 93%;" class="text-right">
    <ul class="pagination" >
        ...
        <li><a href="${APP_PATH }/user/list.do?pageIndex=1" >首页</a></li>
        <li><a href="${APP_PATH }/user/list.do?pageIndex=${pageUtil.pageIndex>1?pageUtil.pageIndex-1:1}" class='pre'>上一页</a></li>

        <c:forEach begin="1" end="${pageUtil.pageCount}" var="i">
            <li><a href="${APP_PATH }/user/list.do?pageIndex=${i}" style="text-decoration: none;">${i}</a></li>
        </c:forEach>

        <li><a href="${APP_PATH }/user/list.do?pageIndex=${pageUtil.pageIndex<pageUtil.pageCount?pageUtil.pageIndex+1:pageUtil.pageCount}"  class='next'>下一页</a></li>
        <li><a href="${APP_PATH }/user/list.do?pageIndex=${pageUtil.pageCount}" class='last' >末页</a></li>
    </ul>
</div>
</div>
